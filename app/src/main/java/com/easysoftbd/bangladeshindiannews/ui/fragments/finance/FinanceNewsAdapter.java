...
public class FinanceNewsAdapter extends RecyclerView.Adapter<FinanceNewsAdapter.FinanceViewHolder> implements ItemClickListener {

    private Context context;
    private List<RecyclerItemModel> list;
    private HomeActivity homeActivity;

    public FinanceNewsAdapter(Context context, List<RecyclerItemModel> list) {
...
//        holder.binding.titleTextView.setTextColor(ContextCompat.getColor(context,getColorCode(list.get(position).getTextColor())));
        ...
        holder.binding.marqueeMoreOptionImageView.setOnClickListener(view -> {
            homeActivity.showFinanceMoreOption(list.get(position).getSerialNumber());
        });
    ...
    static class FinanceViewHolder extends RecyclerView.ViewHolder{
        ...
        public FinanceViewHolder(@NonNull RecyclerViewModelLayoutBinding binding) {
    ...
    public void onMarqueeItemClickListener(String tag) {
        for (int i=0; i<list.size(); i++) {
            if (tag.equalsIgnoreCase(list.get(i).getTitle())) {
                homeActivity.showFinanceAlertDialog(list.get(i).getNewsAndLinkModelList());
            }
        }
    }
